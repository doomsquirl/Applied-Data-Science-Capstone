...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df[filtered_df['Launch Site'] == entered_site].groupby(['class']).count().reset_index().rename({'Launch Site': 'count'}, axis=1),
        values='count', 
        names='class', 
...
def get_scatter_chart(entered_site, entered_mass):
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(entered_mass[0], entered_mass[1])]
    if entered_site == 'ALL':
        # return the outcomes scatter plot for all sites
